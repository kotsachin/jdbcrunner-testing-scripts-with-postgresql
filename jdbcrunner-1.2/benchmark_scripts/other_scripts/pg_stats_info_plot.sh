...
date=$1
#echo "in pg_statsinfo_plot"
#-----------------------------
#--Generate graph for each file in .png format under pgStats directory.
...
EOF
	...
	plotdb1(){
                ...
                yrange='[0:20000]'
                plot=( '"'$file'"'  using 2:\(\$6/1000000\) title '"pgStats with DB_size"' with lines  )
                graph
        }
        plotdb1
        mv db_size.png $f/gnuPlot/pgStats/db_size.png
        ...
        plotdb2(){
                ...
                yrange='[0:100]'
                plot=( '"'$file'"'  using 2:\(\$8/1000000\) title '"pgStats with DB_xact_commit"' with lines  )
                graph
        }
        plotdb2
        mv db_xact_commit.png $f/gnuPlot/pgStats/db_xact_commit.png
        ...
        plotdb3(){
                ...
                yrange='[0:200]'
                plot=( '"'$file'"'  using 2:\(\$9/1000\) title '"pgStats with DB_size"' with lines  )
                graph
        }
        plotdb3
                                                                                     
	mv db_xact_rollback.png $f/gnuPlot/pgStats/db_xact_rollback.png
        ...
        plotdb4(){
                ...
                yrange='[0:100]'
                plot=( '"'$file'"'  using 2:\(\$10/1000000\) title '"pgStats with DB_blks_read"' with lines  )
                graph
        }
        plotdb4
                                                                                     
	mv db_blks_read.png $f/gnuPlot/pgStats/db_blks_read.png
        plotdb5(){
                fileType="png"
                ...
                yrange='[0:1000]'
                plot=( '"'$file'"'  using 2:\(\$11/1000000\) title '"pgStats with DB_bulks_hit"' with lines  )
                graph
        }
        plotdb5
                                                                                     
	mv db_bulks_hit.png $f/gnuPlot/pgStats/db_bulks_hit.png
        plotdb6(){
                fileType="png"
                ...
                yrange='[0:1000]'
                plot=( '"'$file'"'  using 2:\(\$12/1000000\) title '"pgStats with DB_tup_returned"' with lines  )
                graph
        }
        plotdb6
	mv db_tup_returned.png $f/gnuPlot/pgStats/db_tup_returned.png
        ...
        plotdb7(){
                ...
                yrange='[0:1000]'
                plot=( '"'$file'"'  using 2:\(\$13/1000000\) title '"pgStats with DB_tup_fetched"' with lines  )
                graph
        }
        plotdb7
                                                                                     
	mv db_tup_fetched.png $f/gnuPlot/pgStats/db_tup_fetched.png
        ...
        plotdb8(){
                ...
                yrange='[0:500]'
                plot=( '"'$file'"'  using 2:\(\$14/1000000\) title '"pgStats with DB_tup_inserted"' with lines  )
                graph
        }
        plotdb8
                                                                                     
	mv db_tup_inserted.png $f/gnuPlot/pgStats/db_tup_inserted.png
        ...
        plotdb9(){
                ...
                yrange='[0:200]'
                plot=( '"'$file'"'  using 2:\(\$15/1000000\) title '"pgStats with DB_tup_updated"' with lines  )
                graph
        }
        plotdb9
                                                                                     
	mv db_tup_updated.png $f/gnuPlot/pgStats/db_tup_updated.png
        ...
        plotdb10(){
                ...
                yrange='[0:20]'
                plot=( '"'$file'"'  using 2:\(\$16/1000000\) title '"pgStats with DB_tup_deleted"' with lines  )
                graph
        }
        plotdb10
                                                                                     
	mv db_tup_deleted.png $f/gnuPlot/pgStats/db_tup_deleted.png
...
EOF
        ...
        plotproc
        mv processes_usage.png $f/gnuPlot/pgStats/processes_usage.png
...
EOF
        ...
        plotxlog
        mv xlog_stats.png $f/gnuPlot/pgStats/xlog_stats.png
...
EOF
        ...
        plot_table1
        mv table_size.png $f/gnuPlot/pgStats/table_size.png
        ...
        plot_table2
        mv table_blocks_read.png $f/gnuPlot/pgStats/table_blocks_read.png
        ...
        plot_table3
        mv table_index_read.png $f/gnuPlot/pgStats/table_index_read.png

#        plot_table4(){
#                fileType="png"
#                output='"table_toast_reads.png"'
#                title='"Disk activity of tables in tpcc"'
#                xlabel='"All Tables"'
#                ylabel='"Number of blocks toast reads"'
#                yrange='[0:10]'
#                plot=( '"'$file'"'  using 7:xtic\(3\) title '"table size"'   )
#                graph
#        }
#        plot_table4
#        mv table_toast_reads.png $f/gnuPlot/pgStats/table_toast_reads.png
