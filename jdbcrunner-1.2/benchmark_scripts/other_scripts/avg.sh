...
"=============Swap usage:-=========== \n swap_u: "sused/NR "\n","swap_f: " sfree/NR"\n", \
"=============Disk utilization:-========== \n util: " util/NR }'  $file  > $f/dstat_avg_result.txt
 #awk '{usr+=$2;sys+=$3; idl+=$4; iowait+=$5; hiq+=$6; siq+=$7;recv1+=$8; send1+=$9; } END { print usr/NR,sys/NR,idle/NR,wait/NR,hiq/NR,siq/NR,recv1/NR,send1/NR }'  $file  >> ./benchmark/$date/$server/$run/DSTAT_AVG_RESULT.txt
...
for file in ./benchmark/*/*/*/gnuInput/stat_driver.dat
        do
		path=`dirname $file`
                f=${path%/*}
...
"=================Load average:-============ \n For last 1 minute: " m1/NR "\n" ,"For last 5 minutes: " m5/NR "\n","For last 15 minutes: " m15/NR"\n", \
"=============Swap usage:-=========== \n swap_u: "sused/NR "\n","swap_f: " sfree/NR"\n", \
"=============Disk utilization:-========== \n util: " util/NR }'  $file  > $f/dstat_driver__avg_result.txt
 #awk '{usr+=$2;sys+=$3; idl+=$4; iowait+=$5; hiq+=$6; siq+=$7;recv1+=$8; send1+=$9; } END { print usr/NR,sys/NR,idle/NR,wait/NR,hiq/NR,siq/NR,recv1/NR,send1/NR }'  $file  >> ./benchmark/$date/$server/$run/DSTAT_AVG_RESULT.txt
